...
void ResultRepository::insert(const Result& r) {
    SQLite::Statement q(_db,
        "INSERT INTO result (date, position, votes, proportion, candidate_id) VALUES (?, ?, ?, ?, ?)");
    q.bind(1, r.date);
    q.bind(2, r.position);
    q.bind(3, r.votes);
    q.bind(4, r.proportion);
    q.bind(5, r.candidate_id);
    q.exec();
}

void ResultRepository::update(const Result& r) {
    SQLite::Statement q(_db,
        "UPDATE result SET date=?, position=?, votes=?, proportion=?, candidate_id=? WHERE id=?");
    q.bind(1, r.date);
    q.bind(2, r.position);
    q.bind(3, r.votes);
    q.bind(4, r.proportion);
...
Result ResultRepository::find_by_id(int id) {
    SQLite::Statement q(_db,
        "SELECT id, date, position, votes, proportion, candidate_id FROM result WHERE id = ?");
    q.bind(1, id);
    if (q.executeStep())
        return { q.getColumn(0), q.getColumn(1).getString(),
                 q.getColumn(2), q.getColumn(3), q.getColumn(4).getDouble(), q.getColumn(5) };
...
std::vector<Result> ResultRepository::find_by_date(const std::string& date) {
    SQLite::Statement q(_db,
        "SELECT id, date, position, votes, proportion, candidate_id "
...
std::vector<Result> ResultRepository::find_all() {
    SQLite::Statement q(_db,
        "SELECT id, date, position, votes, proportion, candidate_id FROM result");
    std::vector<Result> result;
    while (q.executeStep())
        result.push_back({ q.getColumn(0), q.getColumn(1).getString(),
                           q.getColumn(2), q.getColumn(3), q.getColumn(4).getDouble(), q.getColumn(5) });
